describe("Request", function () {
  ...
  it("Intercept simple", () => {
    ...
    /*
    cy.wait("@bulbasaur").then((interseption) => {
      cy.log(interseption);
      expect(interseption.response.body).to.have.property("name", "bulbasaur");
  ...
  //Muy importante para ver como se comporta la UI
  it("Forzar que falle", function () {
    cy.intercept("GET", "https://pokeapi.co/api/v2/pokemon-species/1", {
      forceNetworkError: true, //simular que el servidor no esta disponible
    }).as("error");

    cy.visit("https://pokedexpokemon.netlify.app/");
    //va a fallar porque esa es la idea
    cy.contains("Bulbasaur")
      .parent()
      .parent()
      .within((element) => {
  ...
  it.only("cambiando body", function () {
    cy.intercept("GET", "https://pokeapi.co/api/v2/pokemon-species/1", {
      statusCode: 200,
      //puse la info de pikachu en vez de bulbasur
      body: {
        base_happiness: 50,
        capture_rate: 190,
        color: {
    ...
    cy.wait("@pikachu").then((interseption) => {
      cy.log(interseption);
      expect(interseption.response.body).to.have.property("name", "pikachu");
      expect(interseption.response.statusCode).to.eq(200);
    });
  });
});
